...
if( isset( $_POST['addFAQ'] ) ) {
    ...
    $name = $phone = $email =$address = $nationality = $state= $local = $city= $under = $id = $code = "";
    ...
    if( !$_POST["id"] ) {
        $idError = "Please enter bet9ja id <br>";
    } else {
        $id = validateFormData( $_POST["id"] );
    }

    if( !$_POST["code"] ) {
        $codeError = "please enter bet9ja code <br>";
    } else {
    ...
    if( $name && $email && $phone && $address && $nationality && $state && $local && $city && $under && $id && $code) {
        
        // create query
        $query = "INSERT INTO branch (`name`,`phone`, `email`, `address`, `country`,`state`, `lga`, `city`,`hierachy`, `under`, `bet9ja_id`,`bet9ja_code`,`rank`) VALUES ('$name', '$phone','$email','$address','$nationality','$state','$local','$city','branch office','$under','$id','$code','6')";
        ...
        if( $result ) {
            ...
            header( "Location: branch_office?alert=success" );
        } else {
...
<form action="<?php echo htmlspecialchars( $_SERVER['PHP_SELF'] ); ?>" method="post" class="row">
        ...
        $query = "SELECT * FROM branch WHERE hierachy = 'hub 1 office' OR hierachy = 'hub2 office' OR hierachy = 'area office' ";
        $result = mysqli_query( $conn, $query );
    ...
    <div class="col-sm-12" id="all" style="display:none">
      ...
      <div class="form-group">  
        ...
        <input type="text" class="form-control input-lg" id="client-name" name="id" value="">
      </div>
      <div class="form-group">  
        <small class="text-danger">* <?php echo isset($codeError) ? $codeError : ''; ?></small>
        <label for="client-name"><h3 class="h3 text-danger">Bet9ja Code</h3></label>
        <input type="text" class="form-control input-lg" id="client-name" name="code" value="">
      </div>
       <div class="form-group">  
