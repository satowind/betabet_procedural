...
if( isset( $_POST['addFAQ'] ) ) {
    ...
    if( $name && $email && $phone && $address && $nationality && $state && $local && $city && $under) {
        ...
        // create query
        $query = "INSERT INTO branch (`name`,`phone`, `email`, `address`, `country`,`state`, `lga`, `city`,`hierachy`, `under`,`rank`) VALUES ('$name', '$phone','$email','$address','$nationality','$state','$local','$city','hub2 office','$under','4')";
        ...
        if( $result ) {
            ...
            header( "Location: hub2_office?alert=success" );
        } else {
...
<form action="<?php echo htmlspecialchars( $_SERVER['PHP_SELF'] ); ?>" method="post" class="row">
   ...
   <label for="client-email"><h3 class="h3 text-danger">Local Government</h3></label> 
    ...
    <div class="col-sm-12">
        <br><br>
            <a href="hub2_office" type="button" class="btn btn-lg btn-default">Cancel</a>
