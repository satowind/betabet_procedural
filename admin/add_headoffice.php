...
if( isset( $_POST['addFAQ'] ) ) {
    ...
    $name = $phone = $email =$address = $nationality = $state= $local = $city=  "";
    ...
    if( $name && $email && $phone && $address && $nationality && $state && $local && $city ) {
        
        // create query
        $query = "INSERT INTO branch (`name`,`phone`, `email`, `address`, `country`,`state`, `lga`, `city`,`hierachy`,`rank`) VALUES ('$name', '$phone','$email','$address','$nationality','$state','$local','$city','head office','1')";
        ...
        if( $result ) {
            ...
            header( "Location: head_office?alert=success" );
        } else {
